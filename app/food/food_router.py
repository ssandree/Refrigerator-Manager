...
from app.food.food_services import create_food, get_all, get_by_id, update_food, delete_food
from app.food.food_schemas import FoodCreate, FoodUpdate, FoodResponse
...
def create(data: FoodCreate, userId=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {"success": True, "message": None, "data": FoodResponse.model_validate(food)}

# Read - 전체 조회
@router.get("", response_model=dict)
def find_all(userId=Depends(get_current_user), db: Session = Depends(get_db)):
    foods = get_all(db, userId)
    return {"success": True, "message": None, "data": [FoodResponse.model_validate(food) for food in foods]}
...
def update(foodId: str, data: FoodUpdate, userId=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {"success": True, "message": None, "data": FoodResponse.model_validate(updated_food)}
